...
printf("got here1\n");
    char outfile[MAX_FLNAME_SIZE];
    FILE *ofp;
    printf("%s %s %d",begoutfile,type,curr_year);
...
printf("gothere2\n");
        ...
        ofp = fopen(outfile, "w");
        if (ofp == NULL) {
            printf("ERROR: null result for outfile path for actual landscape calculation!");
        }
...
void run_actualnongap(char *landscapefile, char *begoutfile, int **nongap_xcrd, int **nongap_ycrd, int curr_year, int *gapgroup, int groupgapnum, int *groupgapsize, int num_species){
 ...
 //   printf("%d\n",count);
    ...
    for(b =0; b<1;b++){ 
        ...
        int num_chars_written = snprintf(outfile, MAX_FLNAME_SIZE, "%s%dnongapexpectedin%sat%dyrs.txt", begoutfile, q, type, curr_year);
        if (num_chars_written >= MAX_FLNAME_SIZE) {
            printf("ERROR: outfile specification for actual landscape calculation is too big!");
        }
